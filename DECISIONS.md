...
Android's final touch/visual inspection found overexposed ground with the desktop lighting values under GLES. Android now uses ambient energy 0.30 and sunlight 0.50 (desktop remains 0.48/0.85) so terrain relief stays legible. A separate iPhone device compile found that the official arm64 template's LC_BUILD_VERSION SDK is 26.1; Xcode 16.4 cannot link its CADynamicRange/MTLTensorDomain symbols. The new device-build entry point fails early with this specific requirement instead of implying the successful simulator build proves device readiness.
