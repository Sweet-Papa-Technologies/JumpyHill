...
parser.add_argument('--profile', type=Path, default=Path('build/ios-store/TREADFALL.mobileprovision'))
parser.add_argument('--output', type=Path, default=Path('build/ios-store/TREADFALL.ipa'))
args = parser.parse_args()
bundle_id = 'com.sweetpapa.treadfall'
team_id = '6Y5SZ2K5XY'
...
source = args.archive / 'Products/Applications/TREADFALL.app'
...
with tempfile.TemporaryDirectory(prefix='treadfall-sign-') as work:
    payload = Path(work) / 'Payload'
    payload.mkdir()
    app = payload / 'TREADFALL.app'
